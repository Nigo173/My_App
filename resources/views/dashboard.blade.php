            ...
            @php
                if (isset($alldata)) {
                    ...
                    foreach ($alldata as $alldatas) {
                        if (!array_key_exists($alldatas->l_Current, $CurrentArray)) {
                            $CurrentArray[$alldatas->l_Current] = 1;
                            $titleArray[$alldatas->l_Current] = $alldatas->t_lTitle;
                        } elseif (array_key_exists($alldatas->l_Current, $CurrentArray)) {
                            $CurrentArray[$alldatas->l_Current] += 1;
                        }
                    }
                    // 搜尋結果標籤統計
                                ...
                                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                                    ...
                                    @if ($trade->onFirstPage())
                                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                                            <span
                                                class="relative inline-flex items-center px-1 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-l-md leading-5"
                                                aria-hidden="true">
                                    ...
                                    @else
                                        <a href="{{ $trade->previousPageUrl() }}" rel="prev"
                                        ...
                                        @if ($i >= $trade->currentPage() - 1 && $i <= $trade->currentPage() + 1)
                                            @if ($i == $trade->currentPage())
                                                <span aria-current="page">
                                                    <span
                                                        class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium badge-success border border-gray-300 cursor-default leading-5">{{ $i }}</span>
                                                </span>
                                            @else
                                                <a href="{{ $trade->url($i) }}"
                                                    class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium text-success bg-white border border-gray-300 leading-5 hover:text-black hover:bg-gray-100 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150"
                                    ...
                                    @if ($trade->currentPage() < $trade->lastPage() - 1)
                                        <a href="{{ $trade->url($trade->lastPage()) }}"
                                            class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium text-success bg-white border border-gray-300 leading-5 hover:text-black hover:bg-gray-100 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150"
                                            aria-label="{{ __('Go to page :page', ['page' => $trade->lastPage()]) }}">
                                            {{ $trade->lastPage() }}
                                        </a>
                                    @endif
                                    ...
                                    @if ($trade->hasMorePages())
                                        <a href="{{ $trade->nextPageUrl() }}" rel="next"
                                    ...
                                    @else
                                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                            <span
                                                class="relative inline-flex items-center px-1 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-r-md leading-5"
                                                aria-hidden="true">
                                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
